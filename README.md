...
2. 下载 `CodexWebRemote-Setup-1.4.6-win-x64.exe`。
3. 双击安装程序，按提示完成安装。
...
1. 下载 `CodexWebRemote-Portable-1.4.6-win-x64.zip`。
2. 完整解压到固定目录。
...
.\build-installer.ps1 -Version 1.4.6
```

安装器回归测试：

```powershell
.\scripts\installer-test.ps1 -SetupPath .\dist\CodexWebRemote-Setup-1.4.6-win-x64.exe
```
