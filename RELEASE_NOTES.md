# Codex Web Remote v2.0.3

## 接管状态可靠性修复

- 修复桌面 Codex 异常中断后遗留 `task_started` 记录，导致 Web 永久误判“有任务正在进行”的问题。
- 仅在桌面会话事件和文件都持续有近期活动时阻止接管；静默的历史遗留状态会自动失效。
- 新增端到端回归：10 分钟前的遗留任务标记不再阻止 Web 接管。

---

# Codex Web Remote v2.0.2

## 任务同步修复

- 接管后任务列表优先显示桌面 App 的正式任务名称，不再被首条消息摘要覆盖。
- 只读快照会核对本地会话文件版本；桌面 App 有新内容时，Web 打开任务即自动重建快照。
- 新增回归验证，覆盖会话文件更新后的只读快照刷新与接管名称优先级。

---

# Codex Web Remote v2.0.1

## 浏览器与无感更新修复

- Web 接管后的网页操作明确使用独立 Playwright Edge 浏览器，不再错误寻找桌面 App 专属的内置浏览器窗口。
- 浏览器采用隔离会话，修复多次工具调用时持久化配置目录被占用、导致无法导航或截图的问题。
- 接管后的新旧任务都会获得浏览器使用指引；Web 状态栏会显示浏览器已配置，并在实际调用时显示正在操作浏览器。
- 新增浏览器端到端回归：导航、点击和截图均有真实验证。
- 完善无感更新链路、版本显示与 PWA 缓存版本，避免 Web 页面继续加载旧资源。

---

# Codex Web Remote v2.0.0

## 多设备体验、同步与可靠性升级

- 桌面 App 修改任务名称后，Web 任务列表会自动同步；删除和归档使用 Codex 官方线程接口，减少残留项目与两端状态不一致。
- 只读或接管过程中允许提前输入并保存草稿，发送操作仍保持锁定；接管完成后无需等待即可继续发送。
- 模型与推理强度显示具体当前值，不再长期显示“默认模型”或“默认强度”。
- 完善共享控制、只读实时输出、历史快照、任务切换缓存、长输出折叠和统一运行状态提示。
- 保留手机、平板、高分辨率桌面布局，改进明暗主题文字对比、输入区和附件上传体验。
- 安装包继续内置完整 Codex、Browser、终端、Node.js 与沙箱运行环境，保证新电脑双击安装后即可使用。
- 继续支持静默覆盖升级、Web 检查更新、账号切换、额度快照与分账号 token 统计。
